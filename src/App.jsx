...
function WalletSection({ score, nickname, setNickname }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!wc) { alert("ウォレットを接続してください"); setTxStatus(""); return; }
  ...
  return (
    <div style={{
      ...
      <input value={nickname} onChange={e => setNickname(e.target.value.slice(0, 12))}
        placeholder="ニックネーム" style={inputStyle} />

      {!isConnected ? (
        <ConnectWallet>
          <ConnectWalletText>ウォレット接続</ConnectWalletText>
        </ConnectWallet>
      ) : isWrongNetwork ? (
        <div>
          ...
          <button
            style={glowBtn("#FF6B6B", "#FF1744")}
            onClick={() => switchChain({ chainId: base.id })}
            disabled={isSwitching}
          >
            {isSwitching ? "切替中..." : "Base に切り替える"}
          </button>
        </div>
      ) : txStatus === "done" ? (
        <div style={{ fontSize: 12, color: "#6BCB77" }}>✓ 登録済み！</div>
      ) : (
        <div>
          ...
          >
            {txStatus === "pending" ? "送信中..." : txStatus === "error" ? "失敗 再試行" : "スコアを登録 🚀"}
          </button>
        </div>
      )}
    </div>
