        ...
        {data && data.map((entry, i) => (
          <div key={i} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: "bold", color: "#fff" }}>{entry.nickname || "Anonymous"}</div>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.8)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#0d0520", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 16, padding: 24, width: 280, textAlign: "center" }}>
        ...
        <button onClick={onClose} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.4)", cursor: "pointer", fontSize: 12 }}>Cancel</button>
      </div>
    </div>
  );
}
...
function WalletSection({ score, nickname, setNickname }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const baseData = encodeFunctionData({ abi: CONTRACT_ABI, functionName: "submitScore", args: [BigInt(score), nickname.trim()] });
  ...
  return (
    <>
      ...
      <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 10, padding: "12px 16px", marginBottom: 16, width: "100%", maxWidth: 240, textAlign: "center" }}>
        ...
        {!isConnected ? (
          <button style={glowBtn("#4D96FF", "#7B61FF")} onClick={() => setShowModal(true)}>Connect Wallet</button>
        ) : isWrongNetwork ? (
          <div>
            <div style={{ fontSize: 11, color: "#FF6B6B", marginBottom: 8 }}>Switch to Base network</div>
            <button style={glowBtn("#FF6B6B", "#FF1744")} onClick={() => switchChain({ chainId: base.id })} disabled={isSwitching}>{isSwitching ? "Switching..." : "Switch to Base"}</button>
          </div>
        ) : txStatus === "done" ? (
          ...
          <div>
            <div style={{ fontSize: 10, color: "#4CC9F0", marginBottom: 8 }}>
              ...
              <span onClick={() => disconnect()} style={{ marginLeft: 8, color: "rgba(255,255,255,0.3)", cursor: "pointer", fontSize: 9 }}>Disconnect</span>
            </div>
            <button style={glowBtn(txStatus === "error" ? "#FF6B6B" : "#C77DFF", txStatus === "error" ? "#FF1744" : "#7B00FF")} onClick={txStatus === "error" ? () => setTxStatus("") : handleSubmit} disabled={txStatus === "pending"}>
...
export default function TileStackGame() {
  ...
  const draw = useCallback(() => {
    ...
    if (s.stack.length > SCROLL_THRESHOLD) {
      ...
      ctx.fillText("x" + s.stack.length, CANVAS_W - 12, 40);
    }
  }, [drawTile]);
  ...
  const placeTile = useCallback(() => {
    ...
    const nd = ni % 2 === 0 ? 1 : -1;
    s.moving = { x: nd === 1 ? -oW : CANVAS_W, w: oW, dir: nd, speed: Math.min(SPEED_INIT + ni * SPEED_INC, 9), grad: pickGrad(ni) };
  }, [draw]);
  ...
  useEffect(() => {
    const onKey = e => {
      if ((e.code === "Space" || e.key === " ") && phase === "playing") { e.preventDefault(); placeTile(); }
    };
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#050510 0%,#0d0520 50%,#100515 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "monospace", color: "#fff", padding: "16px" }}>
      ...
      <div style={{ position: "relative" }}>
        <canvas ref={canvasRef} width={CANVAS_W} height={CANVAS_H}
          style={{ display: "block", borderRadius: 14, boxShadow: "0 0 60px rgba(100,150,255,0.3), 0 0 0 1px rgba(255,255,255,0.08)", cursor: phase === "playing" ? "pointer" : "default" }}
        ...
        {phase === "idle" && (
          <div style={overlayStyle}>
            ...
            <div style={{ fontSize: 14, color: "rgba(255,255,255,0.65)", marginBottom: 6 }}>Stack tiles as high as possible!</div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", marginBottom: 16 }}>Misaligned parts are cut. Miss = Game Over</div>
            <input value={nickname} onChange={e => setNickname(e.target.value.slice(0, 12))} placeholder="Nickname" style={inputStyle} />
